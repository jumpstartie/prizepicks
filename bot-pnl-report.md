...
**As of:** 2026-08-09 ~21:05 UTC (automation cron)  
**Data freshness:** preferred filled equity mark still **STALE (~56h 34m)** — [DO OR DIE](https://cursor.com/agents/bc-2f9f466b-ae51-4aa8-b5f1-be662ba91ecb) (`cursor/kalshi-15m-research-1ecb`) newest filled-444 equity remains **[12:30:53]** `equity=$61.71 cash=$61.71 open=0 closed=444 win%=86 pnl=+57.1847`. Supervising agent still **active** this hour (`lastMessageActivity` **21:02:26Z**, ~0.04h freeze). Transcript **CHANGED** vs 20:05, but **no new Kalshi equity / FILL / SETTLE** after the **[19:40:33] HALTED** mark.  
**Live reset book:** still **HALTED** — newest live mark **[19:40:33]** `equity=$43.65 cash=$39.64 open=2 closed=32 win%=41 pnl=-11.4299 HALTED` (**unchanged** vs 20:05).  
**Abandoned sprint VM:** [Bot $100 target](https://cursor.com/agents/bc-019fd43c-4a8f-76c0-a895-cc8091c29776) still **frozen @00:46** — mark **$60.61** / closed **14 / 43% / −$15.80** / open **0**. ~92.3h stale. Separate reset ledger — **do not merge**.  
**Source:** DO OR DIE + Bot $100 target transcript pulls @21:05 via `batch-fetch-details` (`2026-08-09T21-05-26Z-06fe`). DO OR DIE transcript **CHANGED** (11,496,932 bytes / 106,261 lines; md5 `d985f2d3ea06ab3df515fc42d33a4198`; prior 11,432,590 / 106,067 / `8eea6bb6cae2e7a25998fd9a87556c68`). Growth is **pumpfun/axiom sniper** work (PR #126), not Kalshi trading.  
**Bot status (live Kalshi):** Mid-band EV printer remains **HALTED** at floor **$20** after the 19:29–19:42 reset session. No unhalt / redeploy visible this hour. Agent attention stays on sniper draft PR **#126**.
...
| Metric | Value |
|---|---|
| **Latest filled equity / cash (preferred lifetime)** | **$61.71** / **$61.71** @12:30:53 (**flat on mark**, open **0**) — **STALE ~56.6h** |
...
| **Live ops this hour** | Transcript **CHANGED** (+64 KB sniper); **0** new Kalshi FILL/SETTLE/equity after 19:40 |
| **Cash-save / halt** | **halt if equity ≤ $20** — still **TRIGGERED**; cash_target OFF; SPRINT OFF |
| **Prior report (PR #127 / 20:00)** | **$61.71 / 444 / +$57.18** filled; live **$43.65 HALTED / 32 / −$11.43** |
| **Δ vs prior report (filled book)** | closed **444→444 (+0)**; closed pnl **+$0.00**; equity **+$0.00** |
| **Hourly since ~20:05 cron (filled book)** | **0–0 / $0** on preferred filled counters |
...
| **Agent status** | DO OR DIE **IDLE** but active thru **21:02Z** (~0.04h); Bot $100 target **IDLE** since ~00:46 (~92.3h) |
| **Transcript** | **CHANGED** — **11,496,932** bytes / **106,261** lines; Kalshi marks unchanged; growth = sniper |
...
**Kalshi trading is idle this hour.** After last hour’s reset/mid-band wake (BNB NO loss −$2.09, equity trough $12.57, halt floor $20), there are **no new Kalshi settles or equity prints** after **19:40**. The supervising agent stayed awake (`lastMessageActivity` **21:02Z**) but the +194 transcript lines are **pumpfun/axiom sniper** activity on draft PR **#126**, not the 15m favorite-maker.

Ops read: historical filled book still profitable on paper (**+$57.18 / 444**), but **live cash is still ~$40**, the mid-band reset ledger is still **halted underwater (−$11.43 / ~13–19)**, and attention has pivoted off Kalshi. Further Kalshi PnL needs an unhalt / redeploy.
...
| **@16:16–20:05 Aug 9** | **444** (unchanged) | **86** | **~382–62** | **+$57.18** | filled mark **$61.71**; live reset woke then **HALTED** |
| **@21:05 Aug 9 (this pull)** | **444** (unchanged) | **86** | **~382–62** | **+$57.18** | filled mark still **$61.71**; **live reset** still **$43.65 / $39.64 / HALTED** |
...
| **@21:05 (this pull)** | **32** (unchanged) | **41** | **~13–19** | **−$11.43** | still **$43.65 / $39.64 HALTED** |

Last on-log Aug9 Kalshi settle (from prior hour; none new this hour):
```
...
|---|---|---|---|
| **@00:46 (still frozen)** | **6–8** (43%) | **−$15.80** | high @mark; stale ~92.3h |
...
- Transcript growth is sniper (`BOT` / paper) — not Kalshi 15m settles
...
| **20:05–21:05 Aug 9 (this report)** | filled **0–0**; reset **0–0** | filled **$0**; reset **$0**; still **HALTED @$43.65** | transcript **CHANGED** (sniper only); no new Kalshi tape |
...
|---|---|---|---|---|
| Prior cron ~20:05 | filled $61.71 / $61.71; live $43.65 / $39.64 | filled 444 / 86% / +57.1847; reset 32 / 41% / −11.4299 | — | PR #127; HALTED; sniper PR #126 opened |
| This pull ~21:05 | filled still $61.71; live still **$43.65 / $39.64** | filled 444 / +57.18; reset 32 / −11.43 | filled **+$0.00**; reset **+$0.00** | agent awake on sniper; Kalshi quiet |
...
2. Filled equity mark age **~56h 34m** — lifetime closed W/L/PnL are last confirmed filled, not live MTM.
3. Live cash **~$39.64** / equity **$43.65** diverge hard from stale mark **$61.71**; use live mark for current bankroll.
4. Equity jump **$12.57 → $43.65** while still HALTED with same closed=32 (prior hour) likely reflects mark-to-market / open valuation recovery, not a new settle (no SETTLE after 19:30 in pulls).
5. Mid-band session is still **HALTED** at floor $20; further Kalshi PnL needs an unhalt / redeploy.
6. Agent continues draft PR **#126** (pumpfun/axiom sniper) — Kalshi attention remains diverted; this hour’s transcript delta is sniper-only.
