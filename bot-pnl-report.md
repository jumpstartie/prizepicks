...
**As of:** 2026-08-13 ~11:16 UTC (automation cron)  
**Data freshness:** preferred filled equity mark still **STALE (~142.75h)** — [DO OR DIE](https://cursor.com/agents/bc-2f9f466b-ae51-4aa8-b5f1-be662ba91ecb) (`cursor/kalshi-15m-research-1ecb`) newest filled-444 equity remains **[12:30:53]** `equity=$61.71 cash=$61.71 open=0 closed=444 win%=86 pnl=+57.1847`. Live reset book still **HALTED** at **[19:40:33]** `equity=$43.65 cash=$39.64 open=2 closed=32 win%=41 pnl=-11.4299` (~87.59h). Agent **IDLE** after fresh LMA **10:19:05Z** (~0.95h before pull; was 02:38:44Z last hour). Transcript **CHANGED** vs 10:10 — sniper-only JOEVER mark refresh (**+8,534** bytes / **+66** lines); **no new Kalshi fills**. Sniper **JOEVER** now **~0.90x / ≈ −$2.09** unrealized (improved ~**+$0.96** vs prior −$3.05).  
**Abandoned sprint VM:** [Bot $100 target](https://cursor.com/agents/bc-019fd43c-4a8f-76c0-a895-cc8091c29776) still **frozen @00:46** — mark **$60.61** / closed **14 / 43% / −$15.80** / open **0**. ~178.49h stale. Separate reset ledger — **do not merge**.  
**Source:** DO OR DIE + Bot $100 target transcript pulls @11:16 via `batch-fetch-details` (`2026-08-13T11-16-56Z-b050`). DO OR DIE transcript **CHANGED** vs 10:10 — **12,674,261** bytes / **115,958** lines; md5 `8ea94a66f72689347512b87e1d718f53`. Draft events PR **#126**.  
**Bot status (live Kalshi):** Preferred mid-band reset remains **HALTED** at floor **$20**. Separate **$0.08** micro restart still floor-halted with **closed=0** — do **not** merge into filled 444 or reset 32. Sniper still live in **JOEVER** (monitor running; mark improved this hour).
...
| Metric | Value |
|---|---|
| **Latest filled equity / cash (preferred lifetime)** | **$61.71** / **$61.71** @12:30:53 (**flat on mark**, open **0**) — **STALE ~142.75h** |
...
| **Live ops this hour** | Preferred Kalshi books **flat**; transcript **CHANGED** (sniper-only); agent **IDLE** (LMA **10:19:05Z**) |
| **Cash-save / halt** | preferred reset: **halt if equity ≤ $20** still **TRIGGERED**; micro book still floor-halted at **$0.08** |
| **Prior report (10:10 / PR #209)** | **$61.71 / 444 / +$57.18** filled; live **$43.65 HALTED / 32 / −$11.43** |
| **Δ vs prior report (filled book)** | closed **444→444 (+0)**; closed pnl **+$0.00**; equity **+$0.00** |
| **Hourly since ~10:10 cron (filled book)** | **0–0 / $0** on preferred filled counters |
...
| **Agent status** | DO OR DIE **IDLE** (LMA **10:19:05Z**, ~0.95h); Bot $100 target **IDLE** since ~00:46 (~178.49h) |
| **Transcript** | **CHANGED** — **12,674,261** bytes / **115,958** lines; md5 `8ea94a66f72689347512b87e1d718f53` |
| **Sniper JOEVER (open)** | mark **~$18,711 (0.90x)** / ≈ **−$2.09** unrealized (**+$0.96** vs 10:10 −$3.05) |
...
**Preferred Kalshi books did not trade this hour.** Filled `closed=444` and live reset `closed=32` are unchanged. The DO OR DIE transcript grew only from a sniper JOEVER PnL refresh (no new Kalshi equity prints, fills, or settles).
...
2. **Sniper roll** — ASADO dust position sold earlier; **JOEVER** still held (**1,525,462** tokens). Entry mcap **$20,814**; latest mark **~$18,711 (0.90x, ≈ −$2.09)** on ~$30.81 entry / $28.72 mark. TP **2.8x** / SL **0.45x**; auto-monitor still running. No exit / no new sniper trade; unrealized improved **~+$0.96** vs last hour.
3. Agent LMA advanced to **10:19:05Z** (~0.95h before pull) from prior **02:38:44Z**; status remains **IDLE**.

Ops read: historical filled book still profitable on paper (**+$57.18 / 444**), live Aug-9 cash still ~**$40** and halted underwater (**−$11.43 / ~13–19**), preferred Kalshi hourly deltas still **flat**, and sniper JOEVER mark improved this hour to **≈ −$2.09** unrealized.
...
| **@16:16 Aug 9 – 10:10 Aug 13** | **444** (unchanged) | **86** | **~382–62** | **+$57.18** | filled mark **$61.71**; live reset **HALTED** |
| **@11:16 Aug 13 (this pull)** | **444** (unchanged) | **86** | **~382–62** | **+$57.18** | filled mark still **$61.71**; **live reset** still **$43.65 / $39.64 / HALTED** |
...
| **@10:10 Aug 13** | **32** (unchanged) | **41** | **~13–19** | **−$11.43** | still **$43.65 / $39.64 HALTED** |
| **@11:16 Aug 13 (this pull)** | **32** (unchanged) | **41** | **~13–19** | **−$11.43** | still **$43.65 / $39.64 HALTED** |
...
|---|---|---|---|
| **@00:46 (still frozen)** | **6–8** (43%) | **−$15.80** | high @mark; stale ~178.49h |
...
Sniper (updated this hour — not Kalshi closed PnL):
- **JOEVER held** — **1,525,462** tokens; entry mcap **$20,814**; mark **~$18,711 (0.90x, ≈ −$2.09)** on entry_usd **$30.81** / mark_usd **$28.72**
- Prior report mark was **0.87x / −$3.05** → unrealized Δ ≈ **+$0.96** this hour
- TP **2.8x** / SL **0.45x**; auto-monitor running (`monitor_joever.py`)
...
|---|---|---|---|---|---|
| **Prior cron ~10:10 → this ~11:16** | **0–0** | **+0** (444→444) | **+$0.00** | **+$0.00** ($61.71→$61.71) | preferred Kalshi flat; sniper-only transcript churn |
...
| **Sniper JOEVER (unrealized)** | — | open held | — | mark **+$0.96** vs prior | now **0.90x / −$2.09** |

**Hourly summary:** filled **0–0 / $0**; live reset **0–0 / $0**; abandoned **0–0 / $0**. Sniper JOEVER still open ≈ **−$2.09** unrealized (not Kalshi closed PnL); mark improved vs 10:10.
...
|---|---|---|---|---|---|---|
| **Preferred filled (lifetime)** | **$61.71** | **$61.71** | **444** | **~382–62** | **+$57.18** | STALE ~142.75h |
| **Live reset (mid-band)** | **$43.65** | **$39.64** | **32** | **~13–19** | **−$11.43** | **HALTED** ~87.59h |
| **Micro restart (dust)** | **$0.08** | **$0.08** | **0** | **0–0** | **$0.00** | floor-halted @11:23 |
| **Abandoned Bot $100** | **$60.61** | **$60.61** | **14** | **6–8** | **−$15.80** | frozen ~178.49h |
| **Sniper JOEVER (open)** | mark ~**$18.7k mcap** | — | open | — | ≈ **−$2.09** unrealized | monitored; not Kalshi |

**Bottom line:** Lifetime filled book still **+$57.18 (~382–62 @ 86%)**. Live Kalshi session still **halted underwater (−$11.43)**. Hourly preferred Kalshi deltas all **flat**. Transcript **CHANGED** on sniper-only JOEVER refresh (**0.90x / ≈ −$2.09**, **+$0.96** vs last hour). Agent **IDLE** after LMA **10:19:05Z**.
