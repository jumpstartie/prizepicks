...
**As of:** 2026-08-13 ~13:12 UTC (automation cron)  
**Data freshness:** preferred filled equity mark still **STALE (~144.70h)** — [DO OR DIE](https://cursor.com/agents/bc-2f9f466b-ae51-4aa8-b5f1-be662ba91ecb) (`cursor/kalshi-15m-research-1ecb`) newest filled-444 equity remains **[12:30:53]** `equity=$61.71 cash=$61.71 open=0 closed=444 win%=86 pnl=+57.1847`. Live reset book still **HALTED** at **[19:40:33]** `equity=$43.65 cash=$39.64 open=2 closed=32 win%=41 pnl=-11.4299` (~89.54h). Agent **IDLE** after LMA **10:19:05Z** (~2.90h before pull). Transcript **UNCHANGED** vs 12:16 — **no new Kalshi fills**, no sniper mark refresh. Sniper **JOEVER** still **~0.90x / ≈ −$2.09** unrealized (**+$0.00** vs prior).  
**Abandoned sprint VM:** [Bot $100 target](https://cursor.com/agents/bc-019fd43c-4a8f-76c0-a895-cc8091c29776) still **frozen @00:46** — mark **$60.61** / closed **14 / 43% / −$15.80** / open **0**. ~180.43h stale. Separate reset ledger — **do not merge**.  
**Source:** DO OR DIE + Bot $100 target transcript pulls @13:12 via `batch-fetch-details` (`2026-08-13T13-12-48Z-7f44`). DO OR DIE transcript **UNCHANGED** vs 12:16 — **12,674,261** bytes / **115,958** lines; md5 `8ea94a66f72689347512b87e1d718f53`. Draft events PR **#126**.  
**Bot status (live Kalshi):** Preferred mid-band reset remains **HALTED** at floor **$20**. Separate **$0.08** micro restart still floor-halted with **closed=0** — do **not** merge into filled 444 or reset 32. Sniper still held in **JOEVER** (last known mark unchanged this hour).
...
| Metric | Value |
|---|---|
| **Latest filled equity / cash (preferred lifetime)** | **$61.71** / **$61.71** @12:30:53 (**flat on mark**, open **0**) — **STALE ~144.70h** |
...
| **Prior report (12:16 / PR #211)** | **$61.71 / 444 / +$57.18** filled; live **$43.65 HALTED / 32 / −$11.43** |
| **Δ vs prior report (filled book)** | closed **444→444 (+0)**; closed pnl **+$0.00**; equity **+$0.00** |
| **Hourly since ~12:16 cron (filled book)** | **0–0 / $0** on preferred filled counters |
...
| **Agent status** | DO OR DIE **IDLE** (LMA **10:19:05Z**, ~2.90h); Bot $100 target **IDLE** since ~00:46 (~180.43h) |
| **Transcript** | **UNCHANGED** — **12,674,261** bytes / **115,958** lines; md5 `8ea94a66f72689347512b87e1d718f53` |
| **Sniper JOEVER (open)** | mark **~$18,711 (0.90x)** / ≈ **−$2.09** unrealized (**+$0.00** vs 12:16) |
...
**Preferred Kalshi books did not trade this hour.** Filled `closed=444` and live reset `closed=32` are unchanged. DO OR DIE transcript fingerprint is identical to the 12:16 pull (no new Kalshi equity prints, fills, settles, or sniper mark updates).
...
2. **Sniper roll** — ASADO dust position sold earlier; **JOEVER** still held (**1,525,462** tokens). Entry mcap **$20,814**; last known mark **~$18,711 (0.90x, ≈ −$2.09)** on ~$30.81 entry / $28.72 mark. TP **2.8x** / SL **0.45x**. No exit / no new sniper trade; unrealized **flat** vs last hour (transcript unchanged).
3. Agent LMA still **10:19:05Z** (~2.90h before pull); status remains **IDLE**.

Ops read: historical filled book still profitable on paper (**+$57.18 / 444**), live Aug-9 cash still ~**$40** and halted underwater (**−$11.43 / ~13–19**), preferred Kalshi hourly deltas still **flat**, and sniper JOEVER mark unchanged at **≈ −$2.09** unrealized.
...
| **@16:16 Aug 9 – 12:16 Aug 13** | **444** (unchanged) | **86** | **~382–62** | **+$57.18** | filled mark **$61.71**; live reset **HALTED** |
| **@13:12 Aug 13 (this pull)** | **444** (unchanged) | **86** | **~382–62** | **+$57.18** | filled mark still **$61.71**; **live reset** still **$43.65 / $39.64 / HALTED** |
...
| **@12:16 Aug 13** | **32** (unchanged) | **41** | **~13–19** | **−$11.43** | still **$43.65 / $39.64 HALTED** |
| **@13:12 Aug 13 (this pull)** | **32** (unchanged) | **41** | **~13–19** | **−$11.43** | still **$43.65 / $39.64 HALTED** |
...
|---|---|---|---|
| **@00:46 (still frozen)** | **6–8** (43%) | **−$15.80** | high @mark; stale ~180.43h |
...
- Prior report mark was also **0.90x / −$2.09** → unrealized Δ ≈ **+$0.00** this hour
- TP **2.8x** / SL **0.45x**; last known auto-monitor was running
- No exit / no new sniper buys this hour (transcript unchanged)
...
|---|---|---|---|---|---|
| **Prior cron ~12:16 → this ~13:12** | **0–0** | **+0** (444→444) | **+$0.00** | **+$0.00** ($61.71→$61.71) | preferred Kalshi flat; transcript UNCHANGED |
...
| **Sniper JOEVER (unrealized)** | — | open held | — | mark **+$0.00** vs prior | still **0.90x / −$2.09** |

**Hourly summary:** filled **0–0 / $0**; live reset **0–0 / $0**; abandoned **0–0 / $0**. Sniper JOEVER still open ≈ **−$2.09** unrealized (not Kalshi closed PnL); mark flat vs 12:16.
...
|---|---|---|---|---|---|---|
| **Preferred filled (lifetime)** | **$61.71** | **$61.71** | **444** | **~382–62** | **+$57.18** | STALE ~144.70h |
| **Live reset (mid-band)** | **$43.65** | **$39.64** | **32** | **~13–19** | **−$11.43** | **HALTED** ~89.54h |
| **Micro restart (dust)** | **$0.08** | **$0.08** | **0** | **0–0** | **$0.00** | floor-halted @11:23 |
| **Abandoned Bot $100** | **$60.61** | **$60.61** | **14** | **6–8** | **−$15.80** | frozen ~180.43h |
| **Sniper JOEVER (open)** | mark ~**$18.7k mcap** | — | open | — | ≈ **−$2.09** unrealized | last known open; not Kalshi |

**Bottom line:** Lifetime filled book still **+$57.18 (~382–62 @ 86%)**. Live Kalshi session still **halted underwater (−$11.43)**. Hourly preferred Kalshi deltas all **flat**. Transcript **UNCHANGED** vs 12:16; sniper JOEVER still **0.90x / ≈ −$2.09**. Agent **IDLE** after LMA **10:19:05Z**.
