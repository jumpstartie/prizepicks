...
**As of:** 2026-08-14 ~13:17 UTC (automation cron)  
**Data freshness:** preferred filled equity mark still **STALE (~168.78h)** — [DO OR DIE](https://cursor.com/agents/bc-2f9f466b-ae51-4aa8-b5f1-be662ba91ecb) (`cursor/kalshi-15m-research-1ecb`) newest filled-444 equity remains **[12:30:53]** `equity=$61.71 cash=$61.71 open=0 closed=444 win%=86 pnl=+57.1847`. Live reset book still **HALTED** at **[19:40:33]** `equity=$43.65 cash=$39.64 open=2 closed=32 win%=41 pnl=-11.4299` (~113.62h). Agent **IDLE** after LMA **10:19:05Z** (~26.98h before pull). Transcript **UNCHANGED** vs 12:06 — **no new Kalshi fills**. Sniper **JOEVER** transcript mark still **~0.90x / ≈ −$2.09**; live public refresh this hour **~$16,616 mcap (0.798x) / mark ≈ $25.41 / ≈ −$5.40** (Δ ≈ **−$0.48** vs 12:06 live −$4.92).  
**Abandoned sprint VM:** [Bot $100 target](https://cursor.com/agents/bc-019fd43c-4a8f-76c0-a895-cc8091c29776) still **frozen @00:46** — mark **$60.61** / closed **14 / 43% / −$15.80** / open **0**. ~204.52h stale. Separate reset ledger — **do not merge**.  
**Source:** DO OR DIE + Bot $100 target transcript pulls @13:17 via `batch-fetch-details` (`2026-08-14T13-17-42Z-dd6f`). DO OR DIE transcript **UNCHANGED** vs 12:06 — **12,674,261** bytes / **115,958** lines; md5 `8ea94a66f72689347512b87e1d718f53`. Draft events PR **#126**.  
...
| Metric | Value |
|---|---|
| **Latest filled equity / cash (preferred lifetime)** | **$61.71** / **$61.71** @12:30:53 (**flat on mark**, open **0**) — **STALE ~168.78h** |
...
| **Prior report (12:06 / PR #231)** | **$61.71 / 444 / +$57.18** filled; live **$43.65 HALTED / 32 / −$11.43** |
| **Δ vs prior report (filled book)** | closed **444→444 (+0)**; closed pnl **+$0.00**; equity **+$0.00** |
| **Hourly since ~12:06 cron (filled book)** | **0–0 / $0** on preferred filled counters |
...
| **Agent status** | DO OR DIE **IDLE** (LMA **10:19:05Z**, ~26.98h); Bot $100 target **IDLE** since ~00:46 (~204.52h) |
| **Transcript** | **UNCHANGED** — **12,674,261** bytes / **115,958** lines; md5 `8ea94a66f72689347512b87e1d718f53` |
| **Sniper JOEVER (open)** | transcript **~$18,711 (0.90x) / ≈ −$2.09**; live refresh **~$16,616 (0.798x) / ≈ −$5.40** (Δ ≈ **−$0.48** vs 12:06 live) |
...
**Preferred Kalshi books did not trade since the prior cron.** Filled `closed=444` and live reset `closed=32` are unchanged. DO OR DIE transcript fingerprint is identical to the 12:06 pull (no new Kalshi equity prints, fills, or settles). Gap since last successful report is ~1.19h (12:06 → 13:17).
...
2. **Sniper roll** — ASADO dust position sold earlier; **JOEVER** still held (**1,525,462** tokens). Entry mcap **$20,814**; last transcript mark **~$18,711 (0.90x, ≈ −$2.09)** on ~$30.81 entry / $28.72 mark. Live public refresh @13:17 (DexScreener): mcap **~$16,616 (0.798x)**, mark ≈ **$25.41**, unrealized ≈ **−$5.40** (Δ ≈ **−$0.48** vs 12:06 live −$4.92). TP **2.8x** / SL **0.45x**. No exit / no new sniper trade in the agent transcript.
3. Agent LMA still **10:19:05Z** (~26.98h before pull); status remains **IDLE**.

Ops read: historical filled book still profitable on paper (**+$57.18 / 444**), live Aug-9 cash still ~**$40** and halted underwater (**−$11.43 / ~13–19**), preferred Kalshi deltas since prior cron still **flat**, and sniper JOEVER still open with a slightly worse live mark (~**−$5.40** unrealized).
...
| **@16:16 Aug 9 – 12:06 Aug 14** | **444** (unchanged) | **86** | **~382–62** | **+$57.18** | filled mark **$61.71**; live reset **HALTED** |
| **@13:17 Aug 14 (this pull)** | **444** (unchanged) | **86** | **~382–62** | **+$57.18** | filled mark still **$61.71**; **live reset** still **$43.65 / $39.64 / HALTED** |
...
| **@12:06 Aug 14** | **32** (unchanged) | **41** | **~13–19** | **−$11.43** | still **$43.65 / $39.64 HALTED** |
| **@13:17 Aug 14 (this pull)** | **32** (unchanged) | **41** | **~13–19** | **−$11.43** | still **$43.65 / $39.64 HALTED** |
...
|---|---|---|---|
| **@00:46 (still frozen)** | **6–8** (43%) | **−$15.80** | high @mark; stale ~204.52h |
...
- Live public refresh @13:17 (DexScreener): mcap **~$16,616 (0.798x)** / mark ≈ **$25.41** / unrealized ≈ **−$5.40** (Δ ≈ **−$0.48** vs 12:06 live)
...
|---|---|---|---|---|---|
| **Prior cron ~12:06 → this ~13:17** | **0–0** | **+0** (444→444) | **+$0.00** | **+$0.00** ($61.71→$61.71) | preferred Kalshi flat; transcript UNCHANGED (~1.19h gap) |
...
| **Sniper JOEVER (unrealized)** | — | open held | — | live mark **≈ −$0.48** vs 12:06 live | transcript still **0.90x / −$2.09**; live **0.798x / −$5.40** |

**Hourly summary:** filled **0–0 / $0**; live reset **0–0 / $0**; abandoned **0–0 / $0**. Sniper JOEVER still open; live mark **worse by ~$0.48** (~**−$5.40**, Δ ≈ **−$0.48**) vs last report’s live refresh.

---

## Verdict

DO OR DIE is **IDLE** with an **unchanged** transcript. Preferred lifetime book remains **+$57.18 / 444 closes / 86%** at filled equity **$61.71**. Live Aug-9 reset book remains **HALTED** at **$43.65 / −$11.43 / 32 closes**. Kalshi W/L and closed PnL since the prior cron are **flat**. Sniper JOEVER unrealized live mark ≈ **−$5.40** (Δ ≈ **−$0.48** vs prior live).
