...
**As of:** 2026-08-12 ~12:12 UTC (automation cron)  
**Data freshness:** preferred filled equity mark still **STALE (~119.68h)** — [DO OR DIE](https://cursor.com/agents/bc-2f9f466b-ae51-4aa8-b5f1-be662ba91ecb) (`cursor/kalshi-15m-research-1ecb`) newest filled-444 equity remains **[12:30:53]** `equity=$61.71 cash=$61.71 open=0 closed=444 win%=86 pnl=+57.1847`. Live reset book still **HALTED** at **[19:40:33]** `equity=$43.65 cash=$39.64 open=2 closed=32 win%=41 pnl=-11.4299` (~64.52h). Agent is **IDLE** now (`lastMessageActivity` **11:55:22Z**, ~0.28h before pull) after a busy session: micro Kalshi restart at **$0.08** immediately floor-halted, ASADO sold, and a new sniper long **JOEVER** opened (~0.973x / ≈ −$0.21 unrealized).  
**Abandoned sprint VM:** [Bot $100 target](https://cursor.com/agents/bc-019fd43c-4a8f-76c0-a895-cc8091c29776) still **frozen @00:46** — mark **$60.61** / closed **14 / 43% / −$15.80** / open **0**. ~155.42h stale. Separate reset ledger — **do not merge**.  
**Source:** DO OR DIE + Bot $100 target transcript pulls @12:12 via `batch-fetch-details` (`2026-08-12T12-11-53Z-cd4a`). DO OR DIE transcript **CHANGED** vs 11:21 — **12,657,979** bytes / **115,844** lines; md5 `a7c796ab5f8abb14e23e491a9de18b27` (was 12,086,828 / 111,715 / `97bba4…`). Growth includes new Kalshi micro-restart tape + sniper roll, not new preferred 444/32 equity. Draft events PR **#126**.  
**Bot status (live Kalshi):** Preferred mid-band reset remains **HALTED** at floor **$20**. A separate **$0.08** restart @11:23 also hit the floor immediately (`HALT RUN` / SAVE_BANKROLL / STOPPED) with **closed=0** — do **not** merge into filled 444 or reset 32. Sniper now live in **JOEVER** (monitor running).
...
| Metric | Value |
|---|---|
| **Latest filled equity / cash (preferred lifetime)** | **$61.71** / **$61.71** @12:30:53 (**flat on mark**, open **0**) — **STALE ~119.68h** |
...
| **Live ops this hour** | Transcript **CHANGED**; preferred Kalshi books **flat**; new **$0.08** micro-restart immediately halted; sniper **ASADO→JOEVER** roll; agent **IDLE** (LMA **11:55:22Z**) |
| **Cash-save / halt** | preferred reset: **halt if equity ≤ $20** still **TRIGGERED**; micro book also floor-halted at **$0.08** with cash_target **$60** SAVE armed then STOPPED |
| **Prior report (11:21 / PR #186)** | **$61.71 / 444 / +$57.18** filled; live **$43.65 HALTED / 32 / −$11.43** |
| **Δ vs prior report (filled book)** | closed **444→444 (+0)**; closed pnl **+$0.00**; equity **+$0.00** |
| **Hourly since ~11:21 cron (filled book)** | **0–0 / $0** on preferred filled counters |
...
| **Agent status** | DO OR DIE **IDLE** (LMA **11:55:22Z**, ~0.28h); Bot $100 target **IDLE** since ~00:46 (~155.42h) |
| **Transcript** | **CHANGED** — **12,657,979** bytes / **115,844** lines; md5 `a7c796ab5f8abb14e23e491a9de18b27` (micro-restart + sniper roll + chat) |

Live book source of truth for **lifetime closed W/L + PnL** remains the last filled equity line on **DO OR DIE** (**$61.71 / 444 / +$57.18**). Current live bankroll posture for the Aug-9 mid-band session remains the **halted reset mark** (**$43.65 / $39.64 cash**). The new **$0.08** restart is a separate dust ledger.
...
**Preferred Kalshi books did not trade this hour.** Filled `closed=444` and live reset `closed=32` are unchanged. What *did* happen:
...
2. **Sniper roll** — ASADO dust position sold to raise SOL; **JOEVER** bought (~1.525M tokens / ~0.41 SOL / ~$31) at entry mcap **$20,814**. Latest mark **~$20,250 (0.973x, ≈ −$0.21)**. TP **2.8x** / SL **0.45x**; auto-monitor running. No large realized sniper PnL yet.
3. Agent session freshened LMA to **11:55:22Z** then went **IDLE**.

Ops read: historical filled book still profitable on paper (**+$57.18 / 444**), live Aug-9 cash still ~**$40** and halted underwater (**−$11.43 / ~13–19**), and this hour’s real activity is sniper repositioning plus a failed Kalshi dust restart — not new preferred Kalshi PnL.
...
| **@16:16 Aug 9 – 11:21 Aug 12** | **444** (unchanged) | **86** | **~382–62** | **+$57.18** | filled mark **$61.71**; live reset **HALTED** |
| **@12:12 Aug 12 (this pull)** | **444** (unchanged) | **86** | **~382–62** | **+$57.18** | filled mark still **$61.71**; **live reset** still **$43.65 / $39.64 / HALTED** |
...
| **@11:21 Aug 12** | **32** (unchanged) | **41** | **~13–19** | **−$11.43** | still **$43.65 / $39.64 HALTED** |
| **@12:12 Aug 12 (this pull)** | **32** (unchanged) | **41** | **~13–19** | **−$11.43** | still **$43.65 / $39.64 HALTED** |
...
|---|---|---|---|
| **@00:46 (still frozen)** | **6–8** (43%) | **−$15.80** | high @mark; stale ~155.42h |
...
- **0×** new FILL / LIVE FILL / true SETTLE on preferred books
- **+** micro-restart equity prints at **$0.08** (closed=0) + HALT RUN / SAVE_BANKROLL / STOPPED
...
Sniper (this hour — not Kalshi PnL):
- **ASADO sold** (dust exit to raise SOL; SOL ~0.0009→~0.0046 before larger funding path; no large realized ASADO PnL)
...
- Kalshi preferred books unchanged by the sniper roll
...
|---|---|---|---|---|---|
| **Prior cron ~11:21 → this ~12:12** | **0–0** | **+0** (444→444) | **+$0.00** | **+$0.00** ($61.71→$61.71) | transcript **CHANGED**; preferred Kalshi flat |
| **Live reset same window** | **0–0** | **+0** (32→32) | **+$0.00** | **+$0.00** ($43.65→$43.65) | still **HALTED**; open **2** unchanged |
| **Micro restart ($0.08)** | **0–0** | **+0** (0→0) | **+$0.00** | dust **$0.08** | immediate floor halt; do not merge |
| **Abandoned Bot $100 same window** | **0–0** | **+0** (14→14) | **+$0.00** | **+$0.00** ($60.61) | still frozen; do not merge |

**Hourly summary:** filled **0–0 / $0**; live reset **0–0 / $0**; abandoned **0–0 / $0**. Sniper: ASADO closed (dust), JOEVER open ≈ **−$0.21** unrealized (not Kalshi closed PnL).
...
|---|---|---|---|---|---|
| **Preferred filled (lifetime)** | **$61.71** | **$61.71** | **444** | **~382–62** | **+$57.18** | STALE ~119.68h |
| **Live reset (mid-band)** | **$43.65** | **$39.64** | **32** | **~13–19** | **−$11.43** | **HALTED** ~64.52h |
| **Micro restart (dust)** | **$0.08** | **$0.08** | **0** | **0–0** | **$0.00** | floor-halted @11:23 |
| **Abandoned Bot $100** | **$60.61** | **$60.61** | **14** | **6–8** | **−$15.80** | frozen ~155.42h |
| **Sniper JOEVER (open)** | mark ~**$30.98** | — | open | — | ≈ **−$0.21** unrealized | monitored; not Kalshi |

**Bottom line:** Lifetime filled book still **+$57.18 (~382–62 @ 86%)**. Live Kalshi session still **halted underwater (−$11.43)**. Hourly preferred Kalshi deltas all **flat**. This hour’s change is operational: failed **$0.08** Kalshi restart + sniper roll into **JOEVER** (~0.973x / −$0.21). Agent **IDLE** after LMA **11:55:22Z**.
