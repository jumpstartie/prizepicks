...
**As of:** 2026-08-10 ~05:03 UTC (automation cron)  
**Data freshness:** preferred filled equity mark still **STALE (~64.5h)** — [DO OR DIE](https://cursor.com/agents/bc-2f9f466b-ae51-4aa8-b5f1-be662ba91ecb) (`cursor/kalshi-15m-research-1ecb`) newest filled-444 equity remains **[12:30:53]** `equity=$61.71 cash=$61.71 open=0 closed=444 win%=86 pnl=+57.1847`. Supervising agent is **IDLE** (`lastMessageActivity` **04:38:57Z**, ~0.40h vs pull). Transcript **CHANGED** vs 04:01 — sniper ASADO mark refresh only; **no new Kalshi equity / FILL / SETTLE** after the **[19:40:33] HALTED** mark; ASADO still held with **$0 realized** (mark ~0.96x, unrealized ≈ −$0.01).  
**Live reset book:** still **HALTED** — newest live mark **[19:40:33]** `equity=$43.65 cash=$39.64 open=2 closed=32 win%=41 pnl=-11.4299 HALTED` (**unchanged** vs 04:01).  
**Abandoned sprint VM:** [Bot $100 target](https://cursor.com/agents/bc-019fd43c-4a8f-76c0-a895-cc8091c29776) still **frozen @00:46** — mark **$60.61** / closed **14 / 43% / −$15.80** / open **0**. ~100.3h stale. Separate reset ledger — **do not merge**.  
**Source:** DO OR DIE + Bot $100 target transcript pulls @05:03 via `batch-fetch-details` (`2026-08-10T05-03-06Z-4efb`). DO OR DIE transcript **CHANGED** (11,752,074 bytes / 108,310 lines; md5 `d46fdf3fb3add17b5d2a2e2984f4f3b9`; +14,902 bytes / +116 lines). Draft events PR **#126**.  
**Bot status (live Kalshi):** Mid-band EV printer remains **HALTED** at floor **$20**. No unhalt / redeploy. Sniper position still open; only status refresh this hour.
...
| Metric | Value |
|---|---|
| **Latest filled equity / cash (preferred lifetime)** | **$61.71** / **$61.71** @12:30:53 (**flat on mark**, open **0**) — **STALE ~64.5h** |
...
| **Live ops this hour** | Transcript **CHANGED** (ASADO mark only); **0** new Kalshi FILL/SETTLE/equity after 19:40; sniper still holding ASADO ~**0.96x**, **$0 realized** |
| **Cash-save / halt** | **halt if equity ≤ $20** — still **TRIGGERED**; cash_target OFF; SPRINT OFF |
| **Prior report (PR #135 / 04:01)** | **$61.71 / 444 / +$57.18** filled; live **$43.65 HALTED / 32 / −$11.43** |
| **Δ vs prior report (filled book)** | closed **444→444 (+0)**; closed pnl **+$0.00**; equity **+$0.00** |
| **Hourly since ~04:01 cron (filled book)** | **0–0 / $0** on preferred filled counters |
...
| **Agent status** | DO OR DIE **IDLE** since **04:38Z** (~0.40h); Bot $100 target **IDLE** since ~00:46 (~100.3h) |
| **Transcript** | **CHANGED** — **11,752,074** bytes / **108,310** lines; md5 `d46fdf3fb3add17b5d2a2e2984f4f3b9` |
...
**Kalshi trading is still idle.** After the 19:29–19:42 reset/mid-band wake (BNB NO loss −$2.09, equity trough $12.57, halt floor $20), there are still **no new Kalshi settles or equity prints** after **19:40**. This hour the transcript grew only with an ASADO status check: position still open (`0.00176 SOL` / ~60.9k tokens), mark slipped to **~0.96x** (mcap ~$2,141; unrealized ≈ **−$0.01**), TP 2x / stop-loss off, **no sells**, **no realized sniper PnL**, and **no Kalshi unhalt**. Agent briefly refreshed at **04:38Z** then returned to **IDLE**.
...
| **@16:16 Aug 9 – 04:01 Aug 10** | **444** (unchanged) | **86** | **~382–62** | **+$57.18** | filled mark **$61.71**; live reset **HALTED** |
| **@05:03 Aug 10 (this pull)** | **444** (unchanged) | **86** | **~382–62** | **+$57.18** | filled mark still **$61.71**; **live reset** still **$43.65 / $39.64 / HALTED** |
...
| **@05:03 Aug 10 (this pull)** | **32** (unchanged) | **41** | **~13–19** | **−$11.43** | still **$43.65 / $39.64 HALTED** |
...
|---|---|---|---|
| **@00:46 (still frozen)** | **6–8** (43%) | **−$15.80** | high @mark; stale ~100.3h |
...
- Transcript **CHANGED** only with sniper ASADO status (~+15KB); no new Kalshi tape
...
Sniper (mark refresh only; not Kalshi PnL):
- ASADO all-in still **held** (`0.00176 SOL` / ~60.9k tokens) @ ~**0.96x** (was ~1.03x @04:01)
- Unrealized ≈ **−$0.01** (`cost≈$0.1347` → `mark≈$0.1289`); `asado_sells=0`
- TP 2x / stop-loss off; **no realized sniper PnL**
- Latest note still: insufficient SOL for new live buys (`0.0009` have / `~0.0024` need) — exits-only
...
| **04:01–05:03 Aug 10 (this report)** | filled **0–0**; reset **0–0** | filled **$0**; reset **$0**; still **HALTED @$43.65** | transcript **CHANGED** (ASADO ~1.03x→~0.96x); agent **IDLE** @04:38 |
| **Bot $100 VM 04:01–05:03** | **0–0** | **$0** | still frozen @$60.61 |

### 04:01→05:03 path (this report)

| Step | Equity / cash | Closed / win% / pnl | Δ closed PnL | Notes |
|---|---|---|---|---|
| Prior cron ~04:01 | filled $61.71 / $61.71; live $43.65 / $39.64 | filled 444 / 86% / +57.1847; reset 32 / 41% / −11.4299 | — | PR #135; HALTED; ASADO ~1.03x |
| This pull ~05:03 | filled still $61.71; live still **$43.65 / $39.64** | filled 444 / +57.18; reset 32 / −11.43 | filled **+$0.00**; reset **+$0.00** | sniper mark refresh only; Kalshi quiet; agent IDLE @04:38 |
...
**Hourly sniper:** ASADO still held (~0.96x, was ~1.03x); unrealized ≈ −$0.01; **$0 realized**
...
2. Filled equity mark age **~64.5h** — lifetime closed W/L/PnL are last confirmed filled, not live MTM.
...
6. Agent is **IDLE** after sniper ASADO status refresh @04:38Z (draft PR **#126**); transcript **CHANGED** this hour for sniper mark only; sniper has **no realized PnL** yet (open ~0.96x).
