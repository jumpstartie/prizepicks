...
**As of:** 2026-08-14 ~07:17 UTC (automation cron)  
**Data freshness:** preferred filled equity mark still **STALE (~162.77h)** — [DO OR DIE](https://cursor.com/agents/bc-2f9f466b-ae51-4aa8-b5f1-be662ba91ecb) (`cursor/kalshi-15m-research-1ecb`) newest filled-444 equity remains **[12:30:53]** `equity=$61.71 cash=$61.71 open=0 closed=444 win%=86 pnl=+57.1847`. Live reset book still **HALTED** at **[19:40:33]** `equity=$43.65 cash=$39.64 open=2 closed=32 win%=41 pnl=-11.4299` (~107.61h). Agent **IDLE** after LMA **10:19:05Z** (~20.97h before pull). Transcript **UNCHANGED** vs 06:19 — **no new Kalshi fills**. Sniper **JOEVER** transcript mark still **~0.90x / ≈ −$2.09**; live public refresh this hour **~$17,763 mcap (0.853x) / mark ≈ $26.29 / ≈ −$4.52** (Δ ≈ **$0.00** vs 06:19 live −$4.52).  
**Abandoned sprint VM:** [Bot $100 target](https://cursor.com/agents/bc-019fd43c-4a8f-76c0-a895-cc8091c29776) still **frozen @00:46** — mark **$60.61** / closed **14 / 43% / −$15.80** / open **0**. ~198.50h stale. Separate reset ledger — **do not merge**.  
**Source:** DO OR DIE + Bot $100 target transcript pulls @07:17 via `batch-fetch-details` (`2026-08-14T07-17-00Z-4b8a`). DO OR DIE transcript **UNCHANGED** vs 06:19 — **12,674,261** bytes / **115,958** lines; md5 `8ea94a66f72689347512b87e1d718f53`. Draft events PR **#126**.  
...
| Metric | Value |
|---|---|
| **Latest filled equity / cash (preferred lifetime)** | **$61.71** / **$61.71** @12:30:53 (**flat on mark**, open **0**) — **STALE ~162.77h** |
...
| **Prior report (06:19 / PR #226)** | **$61.71 / 444 / +$57.18** filled; live **$43.65 HALTED / 32 / −$11.43** |
| **Δ vs prior report (filled book)** | closed **444→444 (+0)**; closed pnl **+$0.00**; equity **+$0.00** |
| **Hourly since ~06:19 cron (filled book)** | **0–0 / $0** on preferred filled counters |
...
| **Agent status** | DO OR DIE **IDLE** (LMA **10:19:05Z**, ~20.97h); Bot $100 target **IDLE** since ~00:46 (~198.50h) |
| **Transcript** | **UNCHANGED** — **12,674,261** bytes / **115,958** lines; md5 `8ea94a66f72689347512b87e1d718f53` |
| **Sniper JOEVER (open)** | transcript **~$18,711 (0.90x) / ≈ −$2.09**; live refresh **~$17,763 (0.853x) / ≈ −$4.52** (Δ ≈ **$0.00** vs 06:19 live) |
...
**Preferred Kalshi books did not trade since the prior cron.** Filled `closed=444` and live reset `closed=32` are unchanged. DO OR DIE transcript fingerprint is identical to the 06:19 pull (no new Kalshi equity prints, fills, or settles). Gap since last successful report is ~0.97h (06:19 → 07:17).
...
2. **Sniper roll** — ASADO dust position sold earlier; **JOEVER** still held (**1,525,462** tokens). Entry mcap **$20,814**; last transcript mark **~$18,711 (0.90x, ≈ −$2.09)** on ~$30.81 entry / $28.72 mark. Live public refresh @07:17 (DexScreener): mcap **~$17,763 (0.853x)**, mark ≈ **$26.29**, unrealized ≈ **−$4.52** (Δ ≈ **$0.00** vs 06:19 live −$4.52). TP **2.8x** / SL **0.45x**. No exit / no new sniper trade in the agent transcript.
3. Agent LMA still **10:19:05Z** (~20.97h before pull); status remains **IDLE**.
...
| **@16:16 Aug 9 – 06:19 Aug 14** | **444** (unchanged) | **86** | **~382–62** | **+$57.18** | filled mark **$61.71**; live reset **HALTED** |
| **@07:17 Aug 14 (this pull)** | **444** (unchanged) | **86** | **~382–62** | **+$57.18** | filled mark still **$61.71**; **live reset** still **$43.65 / $39.64 / HALTED** |
...
| **@06:19 Aug 14** | **32** (unchanged) | **41** | **~13–19** | **−$11.43** | still **$43.65 / $39.64 HALTED** |
| **@07:17 Aug 14 (this pull)** | **32** (unchanged) | **41** | **~13–19** | **−$11.43** | still **$43.65 / $39.64 HALTED** |
...
|---|---|---|---|
| **@00:46 (still frozen)** | **6–8** (43%) | **−$15.80** | high @mark; stale ~198.50h |
...
- Live public refresh @07:17: mcap **~$17,763 (0.853x)** / mark ≈ **$26.29** / unrealized ≈ **−$4.52** (Δ ≈ **$0.00** vs 06:19 live)
...
|---|---|---|---|---|---|
| **Prior cron ~06:19 → this ~07:17** | **0–0** | **+0** (444→444) | **+$0.00** | **+$0.00** ($61.71→$61.71) | preferred Kalshi flat; transcript UNCHANGED (~0.97h gap) |
...
| **Sniper JOEVER (unrealized)** | — | open held | — | live mark **≈ $0.00** vs 06:19 live | transcript still **0.90x / −$2.09**; live **0.853x / −$4.52** |
